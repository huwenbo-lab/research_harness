...
研究对象为起点与晋江的作品元数据、作品发现目录与作品日期；叙事编码尚未开始。
按研究者 2026-09-21 的分析需求，当前范围为 `work_date_endpoints`：不再生成逐章明细，
只保留首个可见章、正文起止候选、最后可见章的标题、URL、首次发布与更新时间。
晋江从同一作品详情页提取；起点仍需读取公开目录以确定日期边界，不请求章节正文。
旧 `qidian_chapters` 待办转为 `excluded`，由 `qidian_dates` 接续；已有章节记录原样保留。
正文边界仅依据标题/已提供的卷名和完结状态提出候选，不能替代正文复核；目录覆盖也未独立确认。
起点目录显示的更新时间不会冒充首发时间。该区间描述公开连载证据，不是实际写作起止时间。
当前证据摘要在作品 `metadata_json.publication_window` 中，选中的日期同时进入作品日期表；
正文判断以最新摘要为准，旧候选日期保留来源，不因后续证据不足而被当作已验证结论。
...
- **增量采集**：目录发现作品后直接产生详情任务，重复发现不重复排队。任务类型轮转，
  ...
  首次未处理任务会优先于同优先级的未来刷新任务。
- **两端互不抢任务**：云端仅可领取起点任务，本地仅可领取晋江任务；命令行参数不能
  越过库内分工。每平台串行，请求至少间隔 6 秒，并服从更长的 robots 延时；
  慢响应会延长间隔。robots、会话初始化、重定向都计入预算。
- **失败有状态**：临时断线/5xx 按指数退避，连续 5 次后至少等待一小时，仍可自动重试；
  本地退避上限一天，服务器要求更长等待时仍服从。429、验证页、
  403 等暂停平台。robots 禁止的具体路径不会误停其他允许路径。解析失败保留任务，
  不前移分页。缺库、空库、缺检查点会停止，不自动建立空库覆盖旧状态。
- **自动循环有停止条件**：仅作品详情的 `*_work_metadata_missing` 先做两次间隔重试，
  第三次仍缺失则保留为隔离的 `invalid`，不写入失败结果，也不阻断其他作品。
  同类型连续三个不同作品缺元数据会触发暂停，跨批次保留，目录成功不会清零详情异常。
  未知解析错误、身份冲突、目录异常及内部错误仍须修复。`needs_attention` 表示需告警，
  `halt_required` 表示必须停止；`quarantined_jobs` 单列隔离数量。
  修复后通过明确的 `retry-invalid` 入口重排。
  此入口也可重排尚未隔离的缺元数据重试任务，解除修复后的连续异常暂停；不重排普通网络重试。
...
| `code/crawler_health.py` | 有限校验重试、隔离计数、跨批次异常暂停判定 |
...
| 短暂断网、5xx、限流冷却 | 退避等待，后续批次再尝试；持续失败延长间隔并告警 | 通常不需要 |
| 单本作品缺少元数据 | 有限重试后隔离，其他作品继续；失败结果不入库 | 整体采集不需要重开，隔离作品需复核 |
| 同类多作品连续异常、身份冲突、目录校验失败、数据库或内部错误 | 本地写暂停标记；云端停止相应批次 | 先修复，再恢复/重排 |
...
2026-09-21 作品级采集调整另通过 **226 项测试**（160 crawler、51 cloud、15 local service），
覆盖取消逐章入库、目录日期任务迁移、正文与番外边界候选、首发/更新分离、保留历史数据和云端恢复。

2026-09-24 恢复修复通过 **233 项测试**（163 crawler、55 cloud、15 local service）：
晋江全锁章且缺少评论区的作品页，由点击编号与作品控件的编号、书名、作者交叉核验；
官方目录和详情的空白标题保留作品编号与缺失标记，详情须有显式空白标题节点及匹配身份控件；
云端只读请求最多尝试 3 次，间隔 5、15 秒，每次最长 120 秒。下载在临时目录重试，
成功后才发布到本地目标；权限/不存在错误不反复重试，上传不盲目重试，旧检查点继续保留。

2026-09-26 鲁棒性修复通过 **256 项测试**（173 crawler、63 cloud、20 local service）：
作品 1027343 重新请求后元数据与身份校验均通过，原校验失败没有被当作有效结果入库。
增加缺元数据有限重试、孤立作品隔离、同类型跨批次连续异常暂停；普通网络失败始终保持可重试。
本地、watch 与云端分别报告 `needs_attention` 和 `halt_required`，告警不再自动等同永久停机。
故障页面与恢复验证保存在 `data/raw/crawler_diagnostics_20260926/`、
`data/derived/distributed/recovery_20260926/`；监控必须读取新批次摘要，即使 Actions 显示成功。
...
- `gone` 表示当前页面不可采集，包含 404 和已精确识别的晋江作者锁文、管理员锁定或删除公告，不等于
  研究上已确认作品被删除。保留原作品和失败原因；这类任务目前不自动反复重试，
  作者日后解锁的检测仍需另行恢复任务。
- 2026-09-21 本地恢复修复：新增管理员锁定模板识别；无章节链接的作品页须由
  两个专用作品编号标记和响应 URL 共同核验。8 个异常页面实测重检，7 个通过解析、
  1 个确认当前不可访问；采集与本地服务相关 159 项测试通过。记录保存在
  `data/raw/crawler_diagnostics_20260921/`，最新运行状态仍以后台服务回执为准。
