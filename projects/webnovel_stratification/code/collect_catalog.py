#!/usr/bin/env python3
"""Bounded public catalogs. Stop on access controls; never overwrite a prior batch."""
...
UA='WebnovelBibliographyResearch/0.3 (+https://github.com/huwenbo-lab/research_harness)'
...
def parse_jjwxc(body,source_url,observed_at):
    ...
    for tr in soup.select('table.cytable tr'):
        ...
        raw=clean(td[6].get_text(' ',strip=True));valid=False
        if DATE.fullmatch(raw):
            try: datetime.strptime(raw,'%Y-%m-%d %H:%M:%S');valid=True
            except ValueError: pass
        intro,sep,tags=a.get('title','').partition('标签：');intro=intro.removeprefix('简介：').strip()
        genre=clean(td[2].get_text(' ',strip=True));bits=genre.split('-')
        rows.append(dict(platform='jjwxc',work_id=work_id,title=clean(a.get_text(' ',strip=True)),
          ...
          date_raw=raw,declared_pub_year=int(raw[:4]) if valid else None,
...
def access_problem(body):
    rendered=BeautifulSoup(body,'html.parser').get_text(' ',strip=True)
    if re.search(r'请\s*(?:登入|登录)\s*后再访问',rendered): return 'authentication_required'
    return None

class Collector:
    def __init__(self,out,delay=3):
        if (out/'retrieval_log.json').exists():raise FileExistsError('Use a new output directory; retain prior raw batches')
    ...
    def get(self,url,name,check_robots=True):
        ...
        if check_robots and host not in self.robots:
            ...
            if rb is None:self.robots[host]=None
                ...
                self.delay=max(self.delay,rp.crawl_delay('WebnovelBibliographyResearch') or 0)
        ...
        try:
            with urlopen(Request(url,headers={'User-Agent':UA}),timeout=18) as r:
                ...
                if check_robots and access_problem(data):raise ValueError('authentication_required')
            ...
            if info.get('status') in (202,401,403,429) or any(x in str(e) for x in ['challenge','authentication_required']):self.blocked_hosts.add(host)
            return None
        finally:
    ...
    def jjwxc(self,years,pages_per_year,seed):
        ...
        for year in years:
            if self.blocked_hosts:
                audits.append(dict(year=year,status='not_requested_after_access_control'));continue
            ...
            rows,meta=parse_jjwxc(b,url,self.logs[-1]['retrieved_at'])
            if not rows or {r['declared_pub_year'] for r in rows}!={year}:
                audits.append(dict(year=year,status='annual_filter_unverified',metadata=meta));continue
            ...
            for page in targets:
                if self.blocked_hosts:break
                if page!=1:
                    if not meta['next_url']:
                        audits.append(dict(year=year,page=page,status='no_observed_pagination_url'));continue
                    parts=urlparse(meta['next_url']);q=parse_qs(parts.query,keep_blank_values=True);q['page']=[str(page)]
            ...
            audits.append(dict(year=year,status='complete_catalog' if successes==total else 'partial_page_sample',declared_pages=total,planned_pages=targets,accepted_pages=successes,year_filter_validated=True,seed=seed))
...
def main():
    ...
    ap.add_argument('--pages-per-year',type=int,default=1);ap.add_argument('--seed',type=int,default=20260918);ap.add_argument('--delay',type=float,default=3)
    a=ap.parse_args()
