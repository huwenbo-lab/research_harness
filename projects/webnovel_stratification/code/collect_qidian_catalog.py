...
def main():
    ...
    if a.cat_ids:
        cat_ids=[x.strip() for x in a.cat_ids.split(",") if x.strip().isdigit()]
    elif a.gender=="male":
    ...
    for cat_id in cat_ids:
      for page in range(a.start_page,a.start_page+a.pages):
        params={"catId":cat_id,"pageNum":str(page),"gender":a.gender}
        if csrf: params["_csrfToken"]=csrf
        url=BASE+"?"+urlencode(params)
        referer=f"https://m.qidian.com/category/catid{cat_id}/"
        log={"url":url,"gender":a.gender,"cat_id":cat_id,"page":page,"started_at":utcnow()}
        try:
      ...
      summary={
        "gender":a.gender,"category_ids":cat_ids,"start_page":a.start_page,"requested_pages_per_category":a.pages,
